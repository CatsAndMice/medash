...
test('校验表单', () => {
    ...
    StrategyFromTest.addCacheTest('undefined', [
        { description: 'isEqualsValue:undefined', errorFn: () => console.warn('值不能为undefineded') },
        { description: 'isValueNoEmpty', errorFn: () => console.warn('值不能为空') }
    ]);
    expect(StrategyFromTest.start()).toBeTruthy();
    StrategyFromTest.addCacheTest(testArr, [{
        description: "islengthNoZero",
        errorFn: () => console.warn('数组长度为零')
    }
    ])
    expect(StrategyFromTest.start()).toBeTruthy();
